const lore = {
	...
	conversation: [
		[ //chapter 0, first time they meet, and testing gets unlocked
			() => {
				setTimeout(() => {
					lore.miriam.text("I've never seen this arena before.")
				}, 5000);
			},
			() => {
			...
			() => {
				lore.miriam.text("Does it have control yet?")
			},
			() => {
				lore.miriam.text("I'll authorize it.")
			},
			() => {
				lore.unlockTesting();
				setTimeout(() => {
					lore.miriam.text("Press 'T' and 'U' to go to the next level, among other things.")
				}, 1000);
			},
			() => {
			...
			() => {
				lore.miriam.text("I know, but it does seem to be learning.")
			},
			() => {
				lore.talkingColor = "#dff"
				setTimeout(() => {
					lore.miriam.text("I hope it figures out how to move forward.")
				}, 25000);
			},
			() => {
				lore.talkingColor = "#dff"
